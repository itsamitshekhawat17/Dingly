...
function GallerySection() {
  return (
    <section className="relative left-1/2 w-screen -translate-x-1/2 overflow-hidden px-0">
      {/* <div className="pointer-events-none absolute inset-y-0 left-0 z-10 w-8 bg-linear-to-r from-[#050816] to-transparent sm:w-12" />
      <div className="pointer-events-none absolute inset-y-0 right-0 z-10 w-8 bg-linear-to-l from-[#050816] to-transparent sm:w-12" /> */}

      <div
        className="gallery-marquee flex w-max gap-4 py-2 sm:gap-6"
        style={{ animation: "gallery-marquee 28s linear infinite" }}
      >
        {[...Array(2)].map((_, loopIndex) => (
          <div key={loopIndex} className="flex gap-4 sm:gap-5">
            {Array.from({ length: 6 }).map((_, index) => (
              <article
                key={`${loopIndex}-${index}`}
                className="overflow-hidden rounded-4xl border border-white/10 bg-white/5 p-3 shadow-[0_24px_80px_rgba(2,6,23,0.42)] backdrop-blur"
              >
                <div className="relative h-44 w-40 overflow-hidden rounded-3xl sm:h-56 sm:w-52 lg:h-64 lg:w-60">
                  <Image
                    src={createGalleryDataUri(index)}
                    alt={`Gallery image ${index + 1}`}
                    fill
                    sizes="(max-width: 640px) 10rem, (max-width: 1024px) 13rem, 15rem"
      ...
      <div className="relative w-full px-6 py-14 text-white sm:px-8 lg:px-10 lg:py-16">
        ...
        <div className="mt-10 grid gap-6 lg:grid-cols-2 lg:items-stretch">
          {pricingPlans.map((plan) => (
            <article
              key={plan.name}
              className="flex h-full flex-col rounded-[1.75rem] border border-white/10 bg-white/6 p-5 shadow-[0_16px_40px_rgba(2,6,23,0.22)] sm:p-7"
            >
...
export default function HomePage() {
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#050816] text-white">
      <div className="absolute inset-0 bg-[linear-gradient(rgba(56,189,248,0.08)_1px,transparent_1px),linear-gradient(to_right,rgba(56,189,248,0.08)_1px,transparent_1px)] bg-size-[56px_56px] opacity-35" />
        ...
        <section className="flex flex-1 flex-col justify-center pb-10 pt-8 text-center lg:pt-12">
          <div className="mx-auto max-w-4xl">
            ...
            <h1 className="mx-auto max-w-5xl text-5xl font-semibold tracking-tight text-white sm:text-6xl lg:text-7xl">
              Automate Your Instagram DMs with <span className="text-sky-400">Dingly AI</span>
            </h1>
...
// Tailwind-safe animation support for the sliding image strip.
